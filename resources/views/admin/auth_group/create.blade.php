...
@section('content')
    <div style="margin: 15px;">
        <blockquote class="layui-elem-quote">
            <a href="{{ route('auth_group.index') }}" class="layui-btn layui-btn-small" id="add">
        ...
        <form class="layui-form layui-form-pane" action="">
            ...
            {{ csrf_field() }}
            <div class="layui-form-item">
                <button class="layui-btn" lay-submit lay-filter="addAuthGroup">提 交</button>
            </div>
        </form>
    </div>
@endsection
...
@section('js')
    ...
    <script>
        layui.use(['form', 'layedit', 'laydate', 'upload'], function () {
            ...
            //监听提交
            form.on('submit(addAuthGroup)', function (data) {
                $.post("{{ route('auth_group.store') }}", data.field, function (res) {
                    var msg = '';
                    if (res.code === 0) {
                        msg = '添加成功';
                    } else {
